...
var getOrderById2 = async (req, res) => {
  try {
    const user = req.user;
    const { id } = req.params;
    const result = await orderService.getOrderById(id, user.id, user.role);
    res.status(200).json({
      success: true,
      data: result
    });
